...
export async function processWebhookVideoJob(options = {}) {
  ...
  try {
    ...
    if (localInputPath && fs.existsSync(localInputPath)) {
      ...
      console.log(`[Webhook Pipeline] [1/5] Downloading video from ${url} via yt-dlp...`);
      ...
      while (retries > 0 && !downloadSuccess) {
        try {
          ...
          retries--;
          console.warn(`[Webhook Pipeline] Download retry ${retries} (${client}):`, dlErr.message);
          if (retries === 1) client = 'android';
    ...
    // Extract audio for transcription if needed
    console.log('[Webhook Pipeline] [2/5] Extracting audio for transcription...');
    ...
    const totalDuration = await getDuration(fullVideoPath);
    console.log(`[Webhook Pipeline] Full video duration: ${totalDuration.toFixed(1)}s`);
      ...
      if (process.env.GROQ_API_KEY && process.env.GROQ_API_KEY !== 'your_groq_api_key_here') {
        try {
          ...
          const transcription = await groq.audio.transcriptions.create({
            file: fs.createReadStream(fullAudioPath),
            model: 'whisper-large-v3',
            response_format: 'verbose_json',
            timestamp_granularities: ['word', 'segment'],
          });

          whisperSegments = Array.isArray(transcription.segments) ? transcription.segments : [];
          whisperWords = Array.isArray(transcription.words) ? transcription.words : [];
          transcriptionText = transcription.text || '';
        } catch (whErr) {
          console.warn('[Webhook Pipeline] Groq Whisper transcription warning:', whErr.message);
        }
      }
...
[
  {
    ...
    console.log(`[Webhook Pipeline] [4/5] Rendering ${highlightSegments.length} clip(s)...`);
    const savedClips = [];
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // STEP 4: Dispatch Outbound Webhook Callback
    // ─────────────────────────────────────────────────────────────────────────
    if (callbackUrl) {
