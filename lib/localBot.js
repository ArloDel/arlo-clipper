/**
 * @fileoverview Local Channel Watcher Bot & Background Scheduler for Arlo Clipper.
 * Monitors YouTube channels via XML RSS feeds and yt-dlp, detects newly published videos,
 * and automatically triggers the end-to-end clipper pipeline, generating local MP4 exports,
 * copy text metadata files, and executing automated social media publishing.
 * @module lib/localBot
 */
...
/**
 * Ensures data storage directory and bot config/history JSON files exist on disk.
 * @private
 */
...
/**
 * Bot configuration descriptor.
 * @typedef {typeof DEFAULT_CONFIG} BotConfig
 */

/**
 * Retrieves the current bot watcher configuration from persistent storage.
 *
 * @returns {BotConfig} Active bot configuration
 */
...
/**
 * Saves updated bot configuration and optionally restarts the scheduler timer.
 *
 * @param {Partial<BotConfig>} newConfig - Configuration overrides
 * @param {Object} [options={}] - Options
 * @param {boolean} [options.restartScheduler=true] - Whether to restart interval scheduler
 * @returns {BotConfig} Updated configuration object
 */
...
/**
 * Bot activity log record.
 * @typedef {Object} BotLogEntry
 * @property {string} id - Unique log ID
 * @property {string} timestamp - ISO 8601 timestamp
 * @property {'info'|'warn'|'error'|'success'} level - Severity level
 * @property {string} message - Activity message
 * @property {any} [details] - Detailed metadata
 */

/**
 * Bot history data structure.
 * @typedef {Object} BotHistoryData
 * @property {Array<Object>} processedVideos - List of processed YouTube video records
 * @property {BotLogEntry[]} logs - List of activity log entries
 */

/**
 * Retrieves the complete bot execution history and logs.
 *
 * @returns {BotHistoryData} Bot history object
 */
...
/**
 * Saves bot history and activity logs to disk.
 *
 * @param {BotHistoryData} data - History data to save
 * @returns {BotHistoryData} Saved data object
 */
...
/**
 * Records a new activity log entry for the bot.
 *
 * @param {Object} params
 * @param {'info'|'warn'|'error'|'success'} [params.level='info'] - Severity level
 * @param {string} params.message - Description of event
 * @param {any} [params.details=null] - Additional metadata
 * @returns {BotLogEntry|null} Saved log object
 */
...
/**
 * Decodes XML / HTML character entity references and CDATA blocks into clean Unicode text.
 *
 ...
 *
 * @param {string} urlOrId - Video URL, Short link, or raw ID
...
/**
 * Checks if a YouTube video ID has already been processed or skipped in history.
 *
 * @param {string} videoId - YouTube 11-character Video ID
 * @returns {boolean} True if video is already processed
 */
...
/**
 * Records a newly processed or failed video into the bot history log.
 *
 * @param {Object} videoRecord - Processed video record payload
 * @returns {Object|null} Saved record object
 */
export function addProcessedVideo(videoRecord) {
  try {
    ...
    const rawClips = Array.isArray(videoRecord.clips) ? videoRecord.clips : (existing?.clips || []);
    const enrichedClips = rawClips.map((c) => ({
      ...c,
    ...
    const record = {
      ...
      attempts,
      clipCount: videoRecord.clipCount || enrichedClips.length,
      clips: enrichedClips,
...
/**
 * Clears all bot processed video records and activity history logs.
 *
 * @returns {BotHistoryData} Cleared history structure
 */
...
/**
 * Parses a standard YouTube RSS Atom XML feed into structured video objects.
 *
 * @param {string} xmlString - Raw XML text from YouTube RSS feed
 * @returns {{channel: {channelId: string, channelName: string}, videos: Array<Object>}} Parsed channel & videos
 */
export function parseYouTubeRssFeed(xmlString) {
  if (!xmlString || typeof xmlString !== 'string') return { channel: { channelId: '', channelName: '' }, videos: [] };
...
/**
 * Resolves channel ID, title, RSS feed URL, and recent videos from any channel handle, URL, or ID.
 *
 * @param {string} identifier - Channel URL, @handle, or UC... Channel ID
 * @returns {Promise<{success: boolean, channelId: string, channelName: string, channelUrl: string, rssUrl: string|null, videos: Array<Object>}>} Resolved channel details
 * @throws {Error} If channel cannot be identified or fetched
 */
...
/**
 * Fetches recent video list from a configured YouTube channel.
 *
 * @param {string} channelIdentifier - Channel URL, handle, or ID
 * @returns {Promise<Array<Object>>} List of recent video items
 */
...
/**
 * Generates human-readable TXT and structured JSON metadata files for an exported clip.
 *
 * @param {Object} params
 * @param {Object} params.clip - Generated clip information
 * @param {Object} params.sourceVideo - Source video information
 * @param {string} params.exportFolder - Destination folder path
 * @param {number} [params.clipIndex=1] - Clip index number
 * @returns {{txtPath: string, jsonPath: string, baseName: string, copies: Object}|null} File path details
 */
export function generateClipMetadataFiles({
  ...
  try {
    ...
    const vScore = clip.viralityScore || calculateViralityScore(clip);
...
Virality Score : ${vScore.score}/100 (${vScore.badgeEmoji} ${vScore.gradeLabel} - ${vScore.badge})
...
/**
 * Copies generated MP4 video files and generates TXT/JSON metadata in a dedicated exports directory.
 *
 * @param {Object} params
 * @param {Array<Object>} [params.clips=[]] - Array of clips to export
 * @param {Object} [params.sourceVideo={}] - Source video metadata
 * @param {string} [params.exportDirName='exports'] - Base export folder name
 * @returns {Promise<{exportFolder: string, exportedClips: Array<Object>}>} Export folder and clip paths
 */
...
/**
 * Main Check & Process Engine: Inspects channel, identifies new videos, and runs pipeline.
 *
 * @param {Object} [options={}] - Options
 * @param {boolean} [options.isManual=false] - Manual trigger flag
 * @param {string|null} [options.forceVideoUrl=null] - Direct video URL override
 * @param {string|null} [options.localInputPath=null] - Local video path override
 * @returns {Promise<{success: boolean, processedCount?: number, results?: Array<Object>, message?: string, error?: string}>} Execution outcome
 */
...
/**
 * Starts background polling scheduler (persisting across Next.js dev reloads via globalThis).
 *
 * @returns {boolean} True if scheduler was started, false if disabled
 */
...
/**
 * Stops active background polling scheduler.
 *
...
/**
 * Retrieves the current status of the background scheduler.
 *
 * @returns {{isRunning: boolean, isChecking: boolean, config: BotConfig}} Status object
 */
