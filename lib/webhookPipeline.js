/**
 * @fileoverview Webhook Video Processing Automation Pipeline for Arlo Clipper.
 * End-to-end automated pipeline executing:
 * 1. Video and audio stream downloading / extraction (via youtube-dl-exec and FFmpeg)
 * 2. Speech-to-Text transcription with timestamps (Groq Whisper Large v3)
 * 3. AI Highlight & Viral Moment Detection (Google Gemini Flash & Groq LLM)
 * 4. Framing selection: OpenCV Face Tracking, OpenCV Active Speaker Split Screen, or 9:16 Center Crop
 * 5. Dynamic ASS Subtitle generation with word Karaoke & animations
 * 6. Automated B-Roll detection and insertion
 * 7. Audio ducking and SFX composition
 * 8. Database persistence and outbound callback dispatch with optional social auto-publishing.
 * @module lib/webhookPipeline
 */
...
/**
 * Converts formatted time string ("HH:MM:SS" or "MM:SS") to numeric seconds.
 * @private
 * @param {string|number} timeStr - Time string
...
/**
 * Converts numeric seconds to "HH:MM:SS" format.
 * @private
...
/**
 * Executes OpenCV Face Tracking script (scripts/track_face.py) on a video file.
 * @private
 * @param {Object} params
 * @param {string} params.inputPath - Input video path
 * @param {string} params.outputPath - Target tracked video output path
 * @param {string} [params.ratio='9:16'] - Target aspect ratio
 * @returns {Promise<{success: boolean, error?: string, stdout?: string}>}
 */
...
/**
 * Executes OpenCV Podcast Split Screen script (scripts/track_split_screen.py) on a video file.
 * @private
 * @param {Object} params
 * @param {string} params.inputPath - Input video path
 * @param {string} params.outputPath - Output split screen video path
 * @returns {Promise<{success: boolean, error?: string, stdout?: string}>}
 */
...
/**
 * Options for the automated webhook video processing pipeline.
 * @typedef {Object} ProcessWebhookOptions
 * @property {string} [jobId] - Optional custom job ID
 * @property {string} [url] - Source video HTTP(S) URL
 * @property {string} [localInputPath] - Local source file path for testing/simulation
 * @property {'9:16'|'16:9'|'mobile'|'horizontal'} [ratio='9:16'] - Aspect ratio
 * @property {boolean|string} [faceTracking=false] - Enable OpenCV single face tracking
 * @property {boolean|string} [splitScreen=false] - Enable OpenCV podcast dual split screen
 * @property {boolean|string} [subtitles=true] - Enable ASS hardcoded subtitles
 * @property {'Pop'|'Slide Up'|'Blur'|'Bounce'|'Karaoke'|'None'} [subtitleAnimation='Pop'] - Subtitle animation
 * @property {string} [font='Impact'] - Subtitle font
 * @property {'Small'|'Medium'|'Large'|string} [fontSize='Medium'] - Font size preset
 * @property {string} [color='#FFFF00'] - Subtitle text color hex
 * @property {boolean|string} [broll=true] - Enable B-Roll overlay or theme ID
 * @property {string} [bgm='upbeat-energetic'] - Background music track ID
 * @property {'light'|'medium'|'heavy'|'none'} [ducking='medium'] - Audio ducking preset
 * @property {boolean|string} [sfx=true] - Enable sound effects triggers
 * @property {string} [callbackUrl] - Outbound webhook callback destination
 * @property {string} [secret] - Webhook HMAC signing secret
 * @property {Array<Object>} [customClips] - Custom predefined clip timestamps
 * @property {number} [maxClips=3] - Maximum number of clips to generate
 * @property {{enabled: boolean, platforms: string[], privacy: string}} [autoPublish] - Auto-publish configuration
 */

/**
 * Pipeline processing result report.
 * @typedef {Object} PipelineResult
 * @property {boolean} success - True if pipeline completed successfully
 * @property {string} jobId - Unique job ID
 * @property {'completed'|'failed'} status - Job status
 * @property {number} totalDurationMs - Execution duration in ms
 * @property {Array<Object>} [clips] - Array of generated clip database records
 ...
 *
 * @param {ProcessWebhookOptions} [options={}] - Pipeline options
 * @returns {Promise<PipelineResult>} Complete job outcome
 * @example
 * const result = await processWebhookVideoJob({
