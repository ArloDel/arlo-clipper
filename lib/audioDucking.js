/**
 * @fileoverview Audio Ducking, B-Roll, SFX, and Complex Multi-Stream FFmpeg Filter Builder.
 * Synchronizes video overlay layers, sidechain audio ducking (lowering BGM volume dynamically
 * when vocal speech is active), and trigger-based sound effects with millisecond precision.
 * @module lib/audioDucking
 */
...
/**
 * Sidechain audio compression presets for dynamic ducking.
...
/**
 * Render complex filter configuration output.
 * @typedef {Object} RenderComplexFilterResult
 * @property {string[]} inputs - Array of input file paths for FFmpeg
 * @property {string} filterComplex - Formatted FFmpeg -filter_complex graph string
 * @property {string[]} outputMap - Stream mapping arguments (e.g. ['-map [vout]', '-map [aout]'])
 */

/**
 * Builds the complex filter and multi-stream input list for FFmpeg composition.
 * Combines Video + B-Roll + Subtitles + Main Audio + Ducked BGM + SFX.
 *
 * @param {Object} params
 * @param {string} params.rawVideoPath - Path to base input video
 * @param {string|null} [params.relativeAssPath] - Subtitle .ass file relative path for libass filter
 * @param {number} [params.videoWidth=1080] - Canvas width in pixels
 * @param {number} [params.videoHeight=1920] - Canvas height in pixels
 * @param {number} [params.duration=10] - Target clip duration in seconds
 * @param {Object} [params.audioSettings={}] - Audio mixing configuration
 * @param {string} [params.audioSettings.bgmTrack='none'] - BGM track ID
 * @param {number} [params.audioSettings.bgmVolume=0.3] - BGM volume (0.0 to 1.0)
 * @param {boolean} [params.audioSettings.duckingEnabled=true] - Enable sidechain ducking
 * @param {'light'|'medium'|'heavy'} [params.audioSettings.duckingStrength='medium'] - Ducking strength preset
 * @param {boolean} [params.audioSettings.sfxEnabled=true] - Enable SFX triggers
 * @param {number} [params.audioSettings.sfxVolume=0.7] - SFX volume (0.0 to 1.0)
 * @param {Object} [params.brollSettings={}] - B-Roll overlay configuration
 * @param {boolean} [params.brollSettings.enabled=true] - Enable B-Roll overlays
 * @param {string} [params.brollSettings.theme='auto'] - B-Roll theme ID
 * @param {Array<Object>} [params.brollSettings.overlays=[]] - Array of B-Roll overlay definitions
 * @param {Array<Object>} [params.segments=[]] - Subtitle segments used for SFX hook and transition timing
 * @returns {RenderComplexFilterResult} Prepared FFmpeg inputs and filter complex arguments
 * @example
 * const filterConfig = buildRenderComplexFilter({
 *   rawVideoPath: '/tmp/video.mp4',
 *   relativeAssPath: 'public/clips/sub.ass',
 *   audioSettings: { bgmTrack: 'upbeat-energetic', duckingStrength: 'medium' },
 *   segments: [{ start: 0, end: 3, text: "Hook line" }],
 * });
 */
