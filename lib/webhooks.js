...
// Ensure data folder and webhooks.json exists
...
/**
 * Get configured webhook secret key from environment or storage
 */
...
/**
 * Verify HMAC SHA-256 signature header against payload
 * @param {string|object} payload
...
/**
 * In-memory & disk Webhook activity logger
 */
...
/**
 * Retrieve recent webhook activity logs
 */
...
/**
 * Dispatches an outbound webhook event with retries & HMAC signature
 * @param {object} params
 * @param {string} params.callbackUrl Target webhook URL
 * @param {string} [params.secret] Secret key for signing
 * @param {string} params.event Event name ('clip.completed' | 'clip.failed' | 'test.ping')
 * @param {object} params.data Event payload data
 * @param {number} [params.retries=3] Maximum delivery attempts
 * @param {number} [params.retryDelayMs=1000] Delay between retries in milliseconds
 * @returns {Promise<{success: boolean, deliveryId: string, statusCode?: number, attempts: number, error?: string}>}
 */
